...
def normalizevector(r):  ###
    ...
    denominator = torch.norm(r.reshape(n_batch, -1), p=float('inf'), dim=1, keepdim=True).reshape(shape)
    r /= (denominator + 1e-12)

    denominator = 1e-12 + torch.norm(r.reshape(n_batch, -1), p='fro', dim=1).reshape(shape)
