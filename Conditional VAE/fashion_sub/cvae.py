...
def main():
    ...
    print(args)
    log_dir = args.logdir+'/latent'+str(args.latent_dim)+'_lr'+str(args.lr)+'_kld'+str(args.kld_coef)+'_seed'+str(args.seed)+'_'+str(datetime.now())
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for ep in range(args.epoch):
            ...
            for i in range(args.iter_per_epoch):
                ...
                _ = sess.run(vae.train_step, feed_dict={x:X_train[mask], y:Y_train[mask], lr:decayed_lr, vae.is_train:True})

            test_summary, bce, kld, mse = sess.run([vae.summary, vae.bce_loss, vae.kld_loss, vae.mse_loss], feed_dict={x:X_test[0:200], y:Y_test[0:200], vae.is_train:False})
...
class cVAE(object):
    ...
    def reparamenterize(self, mu, logvar):
        return tf.cond(self.is_train, lambda: tf.random_normal(tf.shape(mu), mean=mu, stddev=tf.exp(0.5*logvar)), lambda: mu)
    ...
    def BCE(self, recon_x, x, mu, logvar):
        return -tf.reduce_mean(tf.reduce_sum(x * tf.log(recon_x+1e-8) + (1-x) * tf.log(1-recon_x+1e-8), axis=1))
