...
parser.add_argument('--numX', type=int, default=200)  # 应该是给标签的data个数
...
def main():
    ...
    print(args)
    log_dir = args.logdir+'_'+str(args.numX)+'/lr'+str(args.lr)+'_epsilon'+str(args.epsilon)+str(args.alpha)+'_seed'+str(args.seed)+'_'+str(datetime.now())
    ...
    Yul_train = Dataset['ytrain']  # y自动是one hot的的
    ...
    for i in range(10):
        print('class:', i, np.sum((Y_train[:, i] == 1)))  # 要求Y_train为one-hot
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for ep in range(args.epoch):
            ...
            for i in range(args.iter_per_epoch):
                ...
                _, loss_ce, loss_vat, loss_en, train_summary = sess.run([train_step, ce_loss, vat_loss, en_loss, train_summary_merged]
