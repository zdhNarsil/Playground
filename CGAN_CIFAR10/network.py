...
class _netD_CIFAR10(nn.Module):
    def __init__(self, zdim, num_classes=10):
        ...
        # num_classes + 1 classification fc
        self.fc_cls = nn.Linear(4 * 4 * 512, num_classes + 1)
        # latent code fc
        self.fc_z = nn.Linear(4 * 4 * 512, self.zdim)
    ...
    def forward(self, input):
        ...
        fc_cls = self.fc_cls(flat6)
        classes = self.softmax(fc_cls)

        fc_z = self.fc_z(flat6)
        z = self.sigmoid(fc_z)
