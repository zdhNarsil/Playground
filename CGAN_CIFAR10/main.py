...
from network import _netD_CIFAR10, _netG_CIFAR10
...
parser.add_argument('--zdim', type=int, default=110, help='size of the latent z vector')
...
try:
    opt.outf = "{0:%Y-%m-%d}_{0:%H-%M-%S}".format(datetime.now())
...
netD = _netD_CIFAR10(num_classes)
...
noise = torch.FloatTensor(opt.batchSize, zdim, 1, 1)
eval_noise = torch.FloatTensor(opt.batchSize, zdim, 1, 1).normal_(0, 1)
...
eval_noise_ = np.random.normal(0, 1, (opt.batchSize, zdim))
...
eval_noise.data.copy_(eval_noise_.view(opt.batchSize, zdim, 1, 1))
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        dis_label.data.resize_(batch_size).fill_(real_label)  # 就是1
        aux_label.data.resize_(batch_size).copy_(label)
        # dis_output, aux_output = netD(input)

        z, classes = netD(real_cpu)
        dis_output, aux_output = 1. - classes[:, 0], classes[:, 1:]  # ?
        ...
        # train with fake
        noise.data.resize_(batch_size, zdim, 1, 1).normal_(0, 1)
        label = np.random.randint(0, num_classes, batch_size)
        noise_ = np.random.normal(0, 1, (batch_size, zdim))
        class_onehot = np.zeros((batch_size, num_classes))
        class_onehot[np.arange(batch_size), label] = 1
        noise_[np.arange(batch_size), :num_classes] = class_onehot[np.arange(batch_size)]
        noise_ = (torch.from_numpy(noise_))
        noise.data.copy_(noise_.view(batch_size, zdim, 1, 1))
        aux_label.data.resize_(batch_size).copy_(torch.from_numpy(label))

        fake = netG(noise)
        dis_label.data.fill_(fake_label)  # 就是0

        # dis_output, aux_output = netD(fake.detach())
        z, classes = netD(fake.detach())  # 为啥detach?
        dis_output, aux_output = 1. - classes[:, 0], classes[:, 1:]  # ?
