...
class advNetwork(tf.keras.Sequential):
    """Network class combining MSA layers
    """
    # 找个地方定义self.eta，初始值就设成0
    def __init__(self, input_shape, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.eta = tf.get_variable(shape=input_shape, initializer=tf.zeros_initializer(),
                                   name='eta', trainable=True)
