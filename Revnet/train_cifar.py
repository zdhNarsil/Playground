...
parser.add_argument("--model", metavar="NAME", help="what model to use")
...
def main():
    ...
    exp_id = "cifar_{0}_{1:%Y-%m-%d}_{1:%H-%M-%S}".format(model.name, datetime.now())
    path = os.path.join("./experiments/", exp_id, "cmd.sh")

    # writer = SummaryWriter(os.path.join("./experiments/", exp_id))
...
def train(epoch, model, criterion, optimizer, trainloader, clip): #, writer):
    model.train()
    train_loss = 0
    correct = 0
    total = 0
    ...
    for i, data in enumerate(t):
        ...
        # writer.add_scalar('Train/Loss', train_loss.item[0], niter)  ## ?
