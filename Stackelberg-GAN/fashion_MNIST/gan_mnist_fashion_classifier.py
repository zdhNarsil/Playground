...
for epoch in tqdm(range(opt.n_epochs)):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        for k in range(opt.n_paths_G):
            ...
            g_loss += F.nll_loss(classifier, target) * 0.1
        ...
        for k in range(opt.n_paths_G):
            ...
            target = Variable(Tensor(imgs.size(0)).fill_(k), requires_grad=False)
            target = target.type(torch.cuda.LongTensor)
            d_loss += F.nll_loss(classifier, target) * 0.1
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(plot_imgs[:100], 'images_ensemble_fashionmnist10/%d.png' % batches_done, nrow=10, normalize=True)
