...
parser.add_argument('--load-path', type=str, default='./MNIST-linear-natural.pt', help='pretrained weight for a simple classifier')
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        ds_size = math.ceil(opt.img_size / 2 ** 4)
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.n_epochs):
        for i, (imgs, labels) in enumerate(dataloader):
            ...
            real_loss = adversarial_loss(discriminator(adv_exps), valid)
    ...
    FGSM_accuracy = AvgMeter()
    IPGD_accuracy = AvgMeter()

    pbar = enumerate(tqdm(test_loader))
    ...
    for j, data in pbar:
        ...
        pred = model(adv_inp)
        acc = torch_accuracy(pred, labels, (1,))
        FGSM_accuracy.update(acc[0].item())

        adv_inp = IPGD(model, images, labels, criterion=criterion, CUDA=CUDA)
        # print(torch.sum(images - adv_inp))
        pred = model(adv_inp)
        acc = torch_accuracy(pred, labels, (1,))
        IPGD_accuracy.update(acc[0].item())
        ...
        pbar_dic['FGSMAcc'] = '{:.2f}'.format(clean_accuracy.mean)
        pbar_dic['IPGDAcc'] = '{:.2f}'.format(clean_accuracy.mean)
