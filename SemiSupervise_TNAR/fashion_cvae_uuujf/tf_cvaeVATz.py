...
from tf_cvae import cVAE
from tf_vat import Net
...
def main():
    ...
    for j in range(1):
        ...
        r_adv_orth = r_adv_orth1 - args.zeta * (
                    tf.reduce_sum(r_x * r_adv_orth, axis=1, keepdims=True) * r_x) + args.zeta * r_adv_orth
    ...
    image_summary_list = [tf.summary.image('x', tf.reshape(x_ul, [-1, 28, 28, 1]), max_outputs=32),
                          ...
                          tf.summary.image('r_adv_orth', tf.reshape(r_adv_orth * args.epsilon2, [-1, 28, 28, 1]),
                                           max_outputs=32),
                          tf.summary.image('x_adv_orth', tf.reshape(x_ul + r_adv_orth * args.epsilon2, [-1, 28, 28, 1]),
                                           max_outputs=32)]
        ...
        for ep in range(args.epoch):
            ...
            for i in range(args.iter_per_epoch):
                ...
                _, loss_ce, loss_vat, loss_vat2, train_summary = sess.run(
                    [train_step, ce_loss, vat_loss, vat_loss_orth, train_summary_merged],
                    feed_dict={x: X_train[mask], y: Y_train[mask], x_ul: Xul_train[mask_ul], lr: decayed_lr,
                               vae.is_train: False})
                # optimize vae
                _, loss_vae = sess.run([vae_train_step, vae_loss],
                                       feed_dict={x_ul: Xul_train[mask_ul], lr: decayed_lr, vae.is_train: True})
